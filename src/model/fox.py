...
class Fox(Fauna):
    ...
    def interact_with_environment(self, i, j,  grid):
        ...
        rabbits_eaten = self.eat_rabbits(i, j, grid)
        if rabbits_eaten > 0 or grid.count_population(Fox) < 2:
            ...
            if self.health > FOX_SOME_REPRODUCTION_THRESHOLD:
                ...
                if grid.count_population(Fox) < (grid.size**2)/4:
                    grid.populate_entities(Fox, num_fox)
        else:
            self.health -= DELTA
